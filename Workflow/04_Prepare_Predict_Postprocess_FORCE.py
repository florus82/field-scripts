...
export_GPU_predictions(predicted_chips_list, 
                    [crop_mask_file for crop_mask_file in getFilelist(f'{prefix}fields/IACS/4_Crop_mask/{year}/', '.tif')], #f'{prefix}fields/IACS/4_Crop_mask/{year}/GSA-DE_BRB-{year}_cropMask_lines_touch_false_lines_touch_false_linecrop.tif', 
                    f'{prefix}fields/Auxiliary/vrt/{year}/{getFORCExyRangeName(get_forcetiles_range(reduced_files))}',
                    row_col_ind, 
                    path_safe(f'{prefix}fields/output/predictions/FORCE/BRANDENBURG/{year}/chips_folder/'),
                    chipsize, overlap)


# # make vrt of predicted image chips
for chip in dirfinder(f'{prefix}fields/output/predictions/FORCE/BRANDENBURG/{year}/chips_folder/'):
    predicted_chips_to_vrt(f'{prefix}fields/output/predictions/FORCE/BRANDENBURG/{year}/chips_folder/', chip,  chipsize, overlap,
                        path_safe(f'{prefix}fields/output/predictions/FORCE/BRANDENBURG/{year}/vrt/'), pyramids=True)
